---
description: Guide to add Bootstrap 4 to Jekyll with focus on having also a css stylesheet using bootstrap variables and custom variables.
---

## Overview

The real power of Bootstrap is in its customizations, it is not
just adding Bootstrap's javascript and stylesheet to use its
components, we need to change and use its variables in our style
sheets also.
...
We define our variables, overwrite the ones we want from Bootstrap
`bower_components/bootstrap/scss/_variables.scss` and then we import
